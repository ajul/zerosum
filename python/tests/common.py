...
class TestBalanceBase(unittest.TestCase):
    strategy_count = 32
    solver_tol = 1e-7
    solution_atol = 1e-6
    num_random_trials = 100

class TestNonSymmetricBalanceBase(TestBalanceBase):
    ...
    def check_result_value(self, result, value, row_weights = None, col_weights = None):
        try:
            ...
            numpy.testing.assert_allclose(numpy.average(result.payoff_matrix, weights = row_weights, axis = 0), value, atol = self.solution_atol)
            numpy.testing.assert_allclose(numpy.average(result.payoff_matrix, weights = col_weights, axis = 1), value, atol = self.solution_atol)
    ...
    def test_check_derivative(self):
        print()
        row_weights = random_weights_with_zeros(self.strategy_count)
        col_weights = random_weights_with_zeros(self.strategy_count + 1)
        kwargs, value = self.generate_random_args(self.strategy_count, self.strategy_count + 1)
    ...
    def test_random_unweighted(self):
        for i in range(self.num_random_trials):
            kwargs, value = self.generate_random_args(self.strategy_count, self.strategy_count + 1)
            result = self.class_to_test(**kwargs).optimize(tol = self.solver_tol)
            self.check_result_value(result, value)
        
    def test_random_weighted(self):
        for i in range(self.num_random_trials):
            row_weights = random_weights(self.strategy_count)
            col_weights = random_weights(self.strategy_count + 1)
            kwargs, value = self.generate_random_args(self.strategy_count, self.strategy_count + 1)
...
class TestSymmetricBalanceBase(TestBalanceBase):
    ...
    def check_result_value(self, result, value, strategy_weights = None):
        try:
            self.assertTrue(result.success)
            numpy.testing.assert_allclose(numpy.average(result.payoff_matrix, weights = strategy_weights, axis = 0), value, atol = self.solution_atol)
            numpy.testing.assert_allclose(numpy.average(result.payoff_matrix, weights = strategy_weights, axis = 1), value, atol = self.solution_atol)
        except Exception as e:
            print(result)
            raise e
    ...
    def test_check_derivative(self):
        print()
        kwargs, value = self.generate_random_args(self.strategy_count)
        strategy_weights = random_weights(self.strategy_count)
    ...
    def test_random_unweighted(self):
        for i in range(self.num_random_trials):
            ...
            result = balance.optimize(tol = self.solver_tol)
            self.check_result_value(result, value)
    ...
    def test_random_weighted_with_zeros(self):
        for i in range(self.num_random_trials):
            strategy_weights = random_weights_with_zeros(self.strategy_count)
            kwargs, value = self.generate_random_args(self.strategy_count)
            balance = self.class_to_test(strategy_weights = strategy_weights, **kwargs)
            result = balance.optimize(tol = self.solver_tol)
            self.check_result_value(result, value, strategy_weights)
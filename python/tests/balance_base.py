...
class TestInitialMatrixBalanceBase(unittest.TestCase):
    """The Balance subclass to be tested."""
    class_to_test = None

    def generate_random_matrix(self, rows, cols):
        raise NotImplementedError

class TestInitialMatrixNonSymmetricBalanceBase(TestInitialMatrixBalanceBase):
    def test_check_derivative(self):
        ...
        data = self.generate_random_matrix(strategy_count, strategy_count + 1)
        result = self.class_to_test(data, row_weights, col_weights, value = value).optimize(check_derivative = True)
    ...
    def test_check_jacobian(self):
        ...
        col_weights = random_weights_with_zeros(strategy_count + 1)
        data = self.generate_random_matrix(strategy_count, strategy_count + 1)
        result = self.class_to_test(data, row_weights, col_weights, value = value).optimize(check_jacobian = True)

    def test_random_unweighted(self):
        for i in range(num_random_trials):
            value = numpy.random.rand() + 1.0
            data = self.generate_random_matrix(strategy_count, strategy_count + 1)
            result = self.class_to_test(data, value = value).optimize(tol = solver_tol)
    ...
    def test_random_weighted(self):
        for i in range(num_random_trials):
            ...
            data = self.generate_random_matrix(strategy_count, strategy_count + 1)
            result = self.class_to_test(data, row_weights, col_weights, value = value).optimize(tol = solver_tol)
